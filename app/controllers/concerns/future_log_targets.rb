...
# Stream that refreshes it after a month-header capture is rendered by
...
module FutureLogTargets
  ...
  # Where a month's ordered entries are refreshed. Any date within the month
  # answers the same id, so the runway's first-of-month and a capture's chosen
  # day both address the same container.
